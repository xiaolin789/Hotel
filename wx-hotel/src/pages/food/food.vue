		...
		<view class="wrap">
			...
			<u-waterfall v-model="flowList" ref="uWaterfall1">
				<template v-slot:left="{leftList}">
					<view @click="toDetails(item.id)" class="demo-warter" v-for="(item, index) in leftList" :key="index">
				...
				<template v-slot:right="{rightList}">
					<view @click="toDetails(item.id)" class="demo-warter" v-for="(item, index) in rightList" :key="index">
						<u-lazy-load threshold="-450" border-radius="10"
							:image="`http://localhost:3000/${item.img_url.split(',')[0]}`" :index="index"></u-lazy-load>
						<view class="demo-title">
							{{item.name}}
