<template>
    <el-main>
      ...
      >
        ...
        <el-table-column  label="预订时间">
            ...
            {{ formatTime.getTime(scope.row.book_time) }}
          </template>
        </el-table-column>
        <el-table-column prop="info_status" label="状态">
          <template #default="scope">
            <el-tag v-if="scope.row.book === 1" class="ml-2" type="success"
              >已预定</el-tag
            >
  ...
  let res = await axios.get('/api/searchuniorder', {params: listParm})
...
const resetBtn = () => {
  ...
  // 分页展示数据
  const getList = async () => {
    let res = await axios.get("/api/uniOrder/list", { params: listParm });
