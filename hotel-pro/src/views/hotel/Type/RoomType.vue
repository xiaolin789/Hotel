<template>
          ...
          <el-row>
            ...
            <el-col :span="12" :offset="0">
              <el-form-item prop="order_num" label="序号">
                <el-input v-model="TypeModel.order_num"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
  ...
  let res = await axios.get('/api/searchtype', {params: listParm})
...
const resetBtn = () => {
  ...
  // 分页展示数据
  const getList = async () => {
    let res = await axios.get("/api/type/list", { params: listParm });
  ...
  const editBtn = async (data) => {
    let res = await axios.get(`/api/type/listId/${data.type_id}`);
    console.log(res.data);
    Object.assign(TypeModel, res.data.data[0]);
    visible.value = true;
  };
    ...
    addFormRef.value.validate(async (valid) => {
      if (valid) {
        let res = await axios.put(
          `/api/type/updateList/${TypeModel.type_id}`,
          TypeModel
        );
  ...
  const deleteBtn = (data) => {
    ElMessageBox.confirm("确定要删除该数据吗", "系统提示", {
      ...
      .then(async () => {
        let res = await axios.delete(`/api/type/list/${data.type_id}`);
        if (res.data.code == "200") {
